...
// 解析参考链接:
// https://blog.csdn.net/fanbird2008/article/details/30075187
// https://hezhaojiang.github.io/post/2021/83dc3ab7/
// https://blog.csdn.net/u011028345/article/details/75267224  get_ref() 推荐
                       ...
                       int i_width, int i_height, const x264_weight_t *weight )
{
    ...
    // mvx & 3 == 3, 表示水平 3/4 像素位

    // 首先取出输入运动矢量亚像素部分（后两位数据），经过计算后赋值给qpel_idx。
    int qpel_idx = ((mvy&3)<<2) + (mvx&3);

    // qpel_idx记录了运动矢量在亚像素单位上指向的位置，它的取值和像素位置之间的关系如下表所示
    ...
    // 补充说明:
    // 上面已经提到这里的16个亚像素分辨率上的点有些是可以直接获取, 比如整像素点和1/2像素点,
    // 但是1/4像素点就需要通过临近的像素点来立即求得, 
    // x264_hpel_ref0与x264_hpel_ref1的两个表就是用来记录当前像素位置需要加载何种分辨率的信息
    // 如果是需要两个指定的相邻像素和合成1/4像素, 两个表就都会有意义,会指向相邻像素的分辨率与方向对应的层
    // 如果不是, 则可能只需要用到x264_hpel_ref0
    // x264_hpel_ref0 与 x264_hpel_ref1 实际上存储的就是当前这个向量指向的亚像素位置的宏块生成需要的1个或者2个参考宏块的分辨率
    // 如果是整像素或者1/2像素就不需要合成,直接从索引中x264_hpel_ref0, 
    // 1/4像素需要两个参考宏块立即现场插值生成

    //src1[]为选择后的半像素数据
    //选择了Full,H,V,C几种“分量”中的1种

    // 获取其中一个平面的像素位
    // x264_hpel_ref0[qpel_idx] 得到平面索引,也就是参考宏块的分辨率
    // src[hpel_ref0[qpel_idx]] + offset 得到该平面对应向量指向的地址
    // 如果当前向量的y分量3/4亚像素位置, 那么src1需要向垂直方向向下移动一行，到下一个整像素点
    // 结合雷霄骅关于x264_macroblock_analyse分析中对get_ref()解析的图可以知道这个操作的原因
    pixel *src1 = src[x264_hpel_ref0[qpel_idx]] + offset + ((mvy&3) == 3) * i_src_stride;

    // qpel_idx & 5 代表了需要两个参考宏块现场进行1/4 像素插值的向量位置
    ...
    if( qpel_idx & 5 ) /* qpel interpolation needed */
    {
        ...
        // src2为第二个参考宏块的相对应的位置
        // 如果当前向量的分量3/4亚像素位置, 那么src2需要向水平方向向右移动一个位置，到下一个整像素点
        pixel *src2 = src[x264_hpel_ref1[qpel_idx]] + offset + ((mvx&3) == 3);
        //进行1/4像素线性内插, 说的直接一点就是求两个宏块之间对应点的平均值
        pixel_avg( dst, *i_dst_stride, src1, i_src_stride,
                   src2, i_src_stride, i_width, i_height );
        // [question]这里加权运动补偿函数的操作没有搞清楚,待研究
        // 大致分析如下:
        // 带权预测，h264在做宏块差值的时候，首先以帧为单位，
        // 计算当前编码帧和被参考帧的平均luma(亮度),chroma(色度)的平均值，
        // 然后计算平均值比值，将这个比值作为权重，
        // 先将参考帧的被参考块做一次比值乘法运算，让其更接近当前编码宏块。
        // 举例来说：参考宏块平均值255,编码宏块平均值130,
        // 先将参考宏块乘以130/255,然后再和编码宏块做差值运算，
        // 这样得到的结果更接近，更容易压缩。
        // 实际上可以理解为加入了均值的考虑, 拉平了亮度上的差异, 有利于编码(能量降低)
        if( weight->weightfn )
            mc_weight( dst, *i_dst_stride, dst, *i_dst_stride, weight, i_width, i_height );
        return dst;
    }
